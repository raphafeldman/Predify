-- ============================================================
-- BASELINE HISTÓRICO — snapshot do supabase/schema.sql aplicado em
-- produção até 2026-07-29. NÃO é uma migration nova: é o ponto de
-- partida da versionagem, criado na Fase 1 (Confiabilidade) do plano
-- de evolução do Predify.
--
-- A partir daqui, nenhuma mudança de schema deve ser mais anexada ao
-- final de supabase/schema.sql — toda alteração nova vira um arquivo
-- de migration próprio nesta pasta (supabase/migrations/), numerado
-- e versionado. schema.sql fica congelado como documentação do
-- estado histórico até este ponto.
--
-- Este arquivo NÃO foi aplicado remotamente por esta tarefa (Fase 1
-- roda só localmente, sem tocar no Supabase de produção) — é cópia
-- fiel do que já está rodando em produção, então reaplicá-lo é
-- seguro (todo o schema.sql já usa "if not exists"/"drop ... create"
-- de forma idempotente), mas não é necessário nem foi executado aqui.
-- ============================================================
